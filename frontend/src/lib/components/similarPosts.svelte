<script>
  ...
  // Process semantic relationships when posts or thread changes
  $: if (similarPosts && similarPosts.length > 0 && thread && thread.tags) {
    processPosts();
  }
  
  // Process posts to add semantic information
  async function processPosts() {
    ...
    try {
      ...
      postsWithSemanticInfo = similarPosts.map(post => ({
        ...post,
        semanticLinks: [],
  ...
  onMount(() => {
    // This ensures we process posts after the component mounts
    ...
    >
      ...
      >
        {#if loadingSemantic}
          ...
          {/each}
        {:else}
          {#each postsWithSemanticInfo as post}
            <div class="px-1.5" style="width: {50 / similarPosts.length}%">
                    ...
                    {#if post.semanticLinks && post.semanticLinks.length > 0}
                      <div class="mb-1 mt-1.5 border-t border-neutral-100 dark:border-neutral-800 pt-1.5">
                        <div class="flex flex-wrap gap-1">
                          ...
                          {#if post.semanticLinks.length > 2}
                            <span class="text-[8px] text-neutral-500">+{post.semanticLinks.length - 2} more</span>
                          {/if}
                        </div>
                      </div>
                    {/if}
