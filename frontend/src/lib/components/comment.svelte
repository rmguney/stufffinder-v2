<script>
  ...
  let showReportCommentModal = false;
  let reportCommentReason = "SPAM";
  let reportCommentNotes = "";
  let reportCommentSubmitting = false;
  let reportCommentError = null;
  let userAlreadyReportedComment = false;
  let commentReports = [];
    ...
    if (currentUser) {
      try {
        const res = await fetch(
          `${PUBLIC_API_URL}/api/reports/COMMENT/${commentId}`,
          {
            headers: {
        ...
        if (res.ok) {
          commentReports = await res.json();
          userAlreadyReportedComment = commentReports.some(
            (report) =>
              report.reporterUsername === currentUser &&
              report.status?.toUpperCase() === "OPEN",
          );
        } else {
          console.warn("Failed to fetch comment reports");
        }
      } catch (err) {
        console.error("Error fetching comment report status:", err);
      }
    }
  });
        ...
        for (let i = 0; i < replyMediaFiles.length; i++) {
          ...
          try {
            ...
            const mediaResponse = await fetch(
              `${PUBLIC_API_URL}/api/comments/${replyId}/upload-media`,
              {
                method: "POST",
            ...
            console.error(
              `Error uploading media file ${i + 1} for reply:`,
              mediaError,
            );
  ...
  const voteOnComment = async (isUpvote) => {
    try {
      ...
      const response = await fetch(
        `${PUBLIC_API_URL}/api/comments/${isUpvote ? "upvote" : "downvote"}/${commentId}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  async function submitCommentReport() {
    if (!reportCommentReason) {
      reportCommentError = "Please select a reason";
      return;
    }

    try {
      reportCommentSubmitting = true;

      const res = await fetch(`${PUBLIC_API_URL}/api/report/submit`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          ...getAuthHeader(),
      ...
      userAlreadyReportedComment = true;
      showReportCommentModal = false;
      reportCommentError = "";
      reportCommentReason = "SPAM";
    } catch (err) {
...
<div class="flex w-full py-0.5">
  <Card.Root
    class="w-full bg-white dark:bg-neutral-950 shadow-sm relative rounded-md
    ${contributingToResolution
      ? 'border-l-4 border-teal-600 dark:border-teal-500'
    ...
    <div class="flex">
      ...
      <div class="flex-1 flex flex-col">
        <Card.Header class="p-4 pb-1">
          ...
          <div
            class="flex flex-wrap items-center gap-2 text-sm text-gray-600 mb-2"
          >
            <a
              href={`/user/${commentator}`}
            ...
            {#if !parentCommentId}
              ...
              {#if commentType === "QUESTION"}
                <span
                  class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-amber-100 dark:bg-amber-900/40 text-amber-800 dark:text-amber-300 border border-amber-200 dark:border-amber-800/50"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
              ...
              {:else if commentType === "SUGGESTION"}
                <span
                  class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-emerald-100 dark:bg-emerald-900/40 text-emerald-800 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800/50"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    class="h-3.5 w-3.5 mr-1.5"
              ...
              {:else}
                <span
                  class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900/40 text-blue-800 dark:text-blue-300 border border-blue-200 dark:border-blue-800/50"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    class="h-3.5 w-3.5 mr-1.5"
            ...
            {#if contributingToResolution}
              <span>•</span>
              <span
                class="text-teal-800 dark:text-teal-400 font-medium flex items-center gap-1"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-4 w-4"
          ...
          {:else}
            <div
              class="text-neutral-900 dark:text-neutral-100 leading-relaxed text-left"
            >
              {comment || "No content"}
            </div>
          {/if}
          ...
          {#if mediaFiles && mediaFiles.length > 0}
            <div
              class="mt-3 bg-neutral-100 dark:bg-neutral-900 rounded-md overflow-hidden"
            >
              <!-- Media carousel -->
              <div class="relative">
                ...
                {#if mediaFiles.length > 1}
                  ...
                  <!-- Media counter -->
                  <div
                    class="absolute bottom-2 left-0 right-0 flex justify-center z-10"
                  >
                    <div
                      class="bg-black bg-opacity-60 text-white px-2 py-0.5 rounded-full text-xs"
                    >
                ...
                <div class="w-full">
                  {#if mediaFiles[currentMediaIndex]}
                    ...
                    {:else if media.type === "audio"}
                      <div
                        class="flex flex-col items-center justify-center p-4 bg-neutral-200 dark:bg-neutral-900"
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          class="h-12 w-12 text-neutral-500 mb-2"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                        >
                    ...
                    {:else}
                      <div
                        class="p-4 bg-neutral-200 dark:bg-neutral-900 text-center"
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          class="h-12 w-12 mx-auto text-neutral-500 mb-2"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                        >
              ...
              {#if mediaFiles.length > 1}
                <div
                  class="flex justify-start overflow-x-auto gap-1 p-1 bg-neutral-200 dark:bg-neutral-900"
                >
                  {#each mediaFiles as media, i}
                    <button
                      on:click|stopPropagation={() => (currentMediaIndex = i)}
                      class="flex-shrink-0 w-12 h-12 rounded overflow-hidden focus:outline-none transition-all {i ===
                      currentMediaIndex
                        ? 'ring-2 ring-neutral-500 transform scale-105'
                    ...
                    >
                      ...
                      {:else if media.type === "video"}
                        <div
                          class="w-full h-full bg-neutral-300 dark:bg-neutral-900 flex items-center justify-center"
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
        ...
        <div
          class="px-4 py-2.5 mt-auto border-t border-neutral-100 dark:border-neutral-800 flex"
        >
          ...
          {#if currentUser && currentUser !== commentator}
            <Button
              variant={userAlreadyReportedComment ? "outline" : "default"}
              size="sm"
              class={`ml-2 text-xs px-3 rounded-full flex items-center transition ${
                userAlreadyReportedComment
                  ? "bg-gray-200 text-gray-500 cursor-not-allowed"
                  : "bg-rose-600 text-white hover:bg-rose-700"
              }`}
              on:click={() => {
                if (!userAlreadyReportedComment) showReportCommentModal = true;
              }}
              disabled={userAlreadyReportedComment}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                ...
                stroke="currentColor"
              >
                <path d="M4 6V20M4 6H15L13.5 9L15 12H4" />
              </svg>
              {userAlreadyReportedComment ? "Reported" : "Report"}
            </Button>
          {/if}
        ...
        {#if replyInputVisible}
          <div class="p-4 pt-3">
            ...
            <div class="my-2 text-left">
              <MediaUploader bind:mediaFiles={replyMediaFiles} />
            </div>
            ...
            <div class="flex justify-end">
              ...
              >
                {#if isUploadingMedia}
                  <span
                    class="inline-block h-3.5 w-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin mr-1.5"
                  ></span>
                  Uploading...
                {:else}
                  <svg
        ...
        {#if replies && replies.length > 0}
          <div
            class="pl-4 border-l-2 border-neutral-200 dark:border-neutral-700 ml-4 mb-3"
          >
  ...
  {#if showReportCommentModal}
    ...
    >
      <div
        class="bg-white dark:bg-neutral-900 rounded-md p-6 w-full max-w-md shadow-lg"
      >
        <h2 class="text-lg font-semibold mb-4">Report Comment</h2>

        {#if reportCommentError}
          <div class="text-red-500 mb-2 text-sm">{reportCommentError}</div>
        {/if}

        <!-- svelte-ignore a11y-label-has-associated-control -->
        <label class="block text-sm mb-1">Reason</label>
        <select
          bind:value={reportCommentReason}
        ...
        <div class="flex justify-end gap-2">
          <Button
            variant="outline"
            on:click={() => (showReportCommentModal = false)}>Cancel</Button
          >
          <Button
            variant="default"
            class="bg-rose-600 text-white hover:bg-rose-700"
            on:click={submitCommentReport}
          >
            Submit
          </Button>
        </div>
...
<style>
  ...
  :global(.dark) .question-comment {
    border-left-color: rgb(
      251 191 36 / 0.6
    ); /* amber-400 at 60% for better visibility */
  }
  ...
  :global(.dark) .story-comment.border-l-4 {
    border-left-color: rgb(
      20 184 166 / 0.8
    ) !important; /* teal-500 at 80% for better visibility */
  }
</style>
