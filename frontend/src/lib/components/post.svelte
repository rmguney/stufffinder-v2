<script>
  ...
  let followerCount = 0;
  let showReportPostModal = false;
  let reportPostReason = "SPAM";
  let reportPostNotes = "";
  let reportPostSubmitting = false;
  let reportPostError = null;
  let userAlreadyReportedPost = false;
  let postReports = [];
  let postId = id;
  ...
  const fetchTagDetails = async () => {
    ...
    try {
      let fetchedTags = await Promise.all(
        tags.map(async (qcode) => {
          if (!qcode) return null;

          try {
            const response = await fetch(
              `https://www.wikidata.org/w/api.php?action=wbgetentities&ids=${qcode}&format=json&languages=en&props=labels|descriptions&origin=*`,
            );
            ...
            return entity
              ? {
                  ...
                  description:
                    entity.descriptions?.en?.value || "No description",
                  id: qcode,
                }
              : null;
          } catch (err) {
  ...
  const resolvePost = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${PUBLIC_API_URL}/api/posts/${id}/resolve`,
        {
          method: "PUT",
          headers: {
  ...
  const unresolvePost = async () => {
    ...
    try {
      const response = await fetch(
        `${PUBLIC_API_URL}/api/posts/${id}/unresolve`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  async function fetchColorHexFromName(colorName) {
    ...
    try {
      const response = await fetch(
        `https://www.thecolorapi.com/id?name=${encodeURIComponent(colorName)}`,
      );
      if (!response.ok) {
  ...
  onMount(async () => {
    ...
    const isAdvancedSearchPage =
      $page.url.pathname.endsWith("/advanced-search");

    if (currentUser && !isAdvancedSearchPage) {
      try {
        const [statusResponse, countResponse, reportsResponse] =
          await Promise.all([
            ...
            fetch(
              `${PUBLIC_API_URL}/api/followed-posts/${id}/followers-count`,
              {
                headers: {
                  "Content-Type": "application/json",
                },
            ...
            fetch(`${PUBLIC_API_URL}/api/reports/POST/${id}`, {
              headers: {
                "Content-Type": "application/json",
                ...getAuthHeader(),
              },
        ...
        try {
          const reportsResponse = await fetch(
            `${PUBLIC_API_URL}/api/reports/POST/${id}`,
            {
              headers: {
                "Content-Type": "application/json",
                ...getAuthHeader(),
          ...
          if (reportsResponse.ok) {
            postReports = await reportsResponse.json();
            userAlreadyReportedPost = postReports.some(
              (report) =>
                report.reporterUsername === currentUser &&
                report.status?.toUpperCase() === "OPEN",
            );
          } else {
            console.warn("Failed to fetch post reports");
          }
        } catch (err) {
          console.error("Error fetching post report status:", err);
        }
      ...
      try {
        ...
        if (!countResponse.ok) throw new Error("Failed to fetch follow data");
  ...
  const handleVote = async (isUpvote) => {
    ...
    try {
      const response = await fetch(
        `${PUBLIC_API_URL}/api/posts/${isUpvote ? "upvote" : "downvote"}/${id}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  async function submitPostReport() {
    ...
    try {
      reportPostSubmitting = true;

      const response = await fetch(`${PUBLIC_API_URL}/api/report/submit`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          ...getAuthHeader(),
      ...
      userAlreadyReportedPost = true;
      showReportPostModal = false;
      reportPostNotes = "";
      reportPostReason = "SPAM";
    } catch (err) {
      reportError = err.message || "Something went wrong.";
    } finally {
      reportPostSubmitting = false;
    }
  }
</script>
...
{#if showResolutionModal}
  ...
  >
    <div
      class="bg-white dark:bg-neutral-950 rounded-md shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-neutral-200 dark:border-neutral-800"
    >
      <div class="p-6">
        ...
        {#if resolutionError}
          <div
            class="bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300 p-3 rounded-md mb-4"
          >
            {resolutionError}
          </div>
        {/if}
        ...
        <div class="mb-4">
          ...
          <div
            id="comment-selection-placeholder"
...
{#if variant === "thumb"}
  ...
  <div
    class="thumb-card flex flex-col bg-white dark:bg-neutral-950 shadow-sm border border-neutral-200 dark:border-neutral-800 rounded-md overflow-hidden h-full transition-all duration-300 hover:shadow-md relative"
  >
    ...
    {:else}
      <div
        class="w-full aspect-[4/3] bg-neutral-100 dark:bg-neutral-800 flex items-center justify-center"
      >
        <svg
    ...
    <div class="p-4 flex flex-col flex-grow">
      <h3
        class="font-medium text-neutral-900 dark:text-white text-lg mb-1 line-clamp-1"
      >
        {title}
      </h3>
      ...
      <p
        class="text-sm text-neutral-600 dark:text-neutral-400 mb-2 line-clamp-1"
      >
        {description}
      </p>
      ...
      {#if $tagDetails && $tagDetails.length > 0}
        <div class="flex flex-wrap gap-1.5 mb-3">
          {#each $tagDetails as tag}
            <span
              class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-neutral-100 dark:bg-neutral-800 text-neutral-800 dark:text-neutral-300 border border-neutral-200 dark:border-neutral-700"
            >
              {tag.label}
            </span>
          {/each}
      ...
      <div
        class="mt-auto pt-3 border-t border-neutral-100 dark:border-neutral-800 flex justify-between items-center"
      >
        <div class="text-xs text-neutral-500 dark:text-neutral-400">
          ...
          {#if solved}
            <span
              class="inline-flex items-center ml-1.5 px-2 py-0.5 rounded-full text-[10px] font-medium bg-teal-100 dark:bg-teal-900/30 text-teal-800 dark:text-teal-300 border border-teal-200 dark:border-teal-800/50"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
          ...
          {:else}
            <span
              class="inline-flex items-center ml-1.5 px-2 py-0.5 rounded-full text-[10px] font-medium bg-rose-100 dark:bg-rose-900/30 text-rose-800 dark:text-rose-300 border border-rose-200 dark:border-rose-800/50"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                class="h-2.5 w-2.5 mr-0.5"
    ...
    {#if variant === "thread"}
      ...
      <div class="flex">
        ...
        <div class="flex-1 p-4">
          ...
          <div class="flex flex-row justify-between text-sm">
            <div>
              ...
              {#if solved}
                <span>•</span>
                <span
                  class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-teal-100 dark:bg-teal-900/30 text-teal-800 dark:text-teal-300 border border-teal-200 dark:border-teal-800/50"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    class="h-3.5 w-3.5 mx-1.5 -ml-0.5"
                    viewBox="0 0 20 20"
                    fill="currentColor"
                  >
                    <path
              ...
              {:else}
                <span>•</span>
                <span
                  class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-rose-100 dark:bg-rose-900/30 text-rose-800 dark:text-rose-300 border border-rose-200 dark:border-rose-800/50"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    class="h-3.5 w-3.5 mx-1.5 -ml-0.5"
              ...
              <span>•</span>
              <span
                class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-neutral-100 dark:bg-neutral-800 text-neutral-700 dark:text-neutral-300 border border-neutral-200 dark:border-neutral-700"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                ...
                <span class="font-medium">{followerCount}</span>
              ...
              {#if $activeUser && $activeUser !== postedBy}
                ...
                {#if !userAlreadyReportedPost}
                  <span>•</span>
                  <button
                    variant="outline"
                    class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium transition-colors border bg-rose-100 dark:bg-rose-900/30 text-rose-800 dark:text-rose-300 border-rose-200 dark:border-rose-800/50 hover:bg-rose-200 dark:hover:bg-rose-900/50"
                    on:click={() => (showReportPostModal = true)}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      class="h-3.5 w-3.5 mr-1"
                    ...
                    >
                      <path
                        fill-rule="evenodd"
                        d="M5 3a1 1 0 011-1h9.586a1 1 0 01.707 1.707L14.414 6l1.879 1.879a1 1 0 01-.707 1.707H6v6a1 1 0 11-2 0V3z"
                        clip-rule="evenodd"
                      />
                    </svg>
                    Report
                ...
                {:else}
                  <button
                    variant="outline"
                    disabled
                    class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium transition-colors border bg-gray-200 text-gray-500 border-gray-300 cursor-not-allowed"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      class="h-3.5 w-3.5 mr-1"
          ...
          <h2
            class="text-xl font-semibold mb-2 text-neutral-900 dark:text-white inline-flex items-center"
          >
            <span>{title}</span>
          </h2>
          ...
          {#if description}
            <div
              class="text-neutral-800 dark:text-neutral-200 mb-1 leading-relaxed whitespace-pre-line"
            >
              {description}
            </div>
          {/if}
      ...
      {#if currentUser === postedBy}
        <div
          class="px-4 py-2.5 border-t border-neutral-100 dark:border-neutral-800"
        >
      ...
      {#if solved && resolution}
        <div class="px-4 pb-5 pt-0">
          <div
            class="bg-teal-50 dark:bg-teal-900/10 border border-teal-200 dark:border-teal-800/50 rounded-md overflow-hidden"
          >
            <div class="px-5 py-4 flex items-start gap-3">
              <div class="mt-1 flex-shrink-0">
                <div
                  class="w-10 h-10 rounded-full bg-teal-100 dark:bg-teal-800/30 flex items-center justify-center text-teal-600 dark:text-teal-400"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    class="h-6 w-6"
              ...
              <div class="flex-1">
                <h3
                  class="font-medium text-md text-teal-800 dark:text-teal-300 mb-2"
                >
                  Mystery Object Resolved
                </h3>
                <div
                  class="text-neutral text-sm 800 dark:text-neutral-200 mb-3 whitespace-pre-line"
                >
                ...
                {#if resolution.contributingComments && resolution.contributingComments.length > 0}
                  <div class="mt-3">
                    <h4
                      class="text-sm font-medium mb-2 text-teal-700 dark:text-teal-400 flex items-center"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        class="h-4 w-4 mr-1"
                    ...
                    {#if contributingCommentObjects && contributingCommentObjects.length > 0}
                      <div
                        class="space-y-2 mt-2 max-h-60 overflow-y-auto border border-teal-200 dark:border-teal-800/50 rounded-md p-2 bg-white/50 dark:bg-neutral-950/50"
                      >
                        {#each contributingCommentObjects as commentObj (commentObj.id)}
                          <div
                            class="p-2 rounded-md bg-white dark:bg-neutral-950 shadow-sm border border-neutral-100 dark:border-neutral-800 hover:border-teal-300 dark:hover:border-teal-700 transition-colors"
                          >
                            <div
                              class="text-xs font-medium mb-1 flex items-center justify-between"
                            >
                              <span
                              ...
                              >
                                ...
                                {#if commentObj.commentType}
                                  <span
                                    class="ml-1.5 px-1.5 py-0.5 rounded-full text-[10px] bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400"
                                  >
                              ...
                              <span class="text-neutral-500 text-[10px]"
                                >{formatDate(commentObj.createdAt)}</span
                              >
                            </div>
                            <p
                              class="text-xs text-neutral-800 dark:text-neutral-200 line-clamp-2"
                            >
                              {commentObj.content}
                            </p>
                          </div>
                    ...
                    {:else}
                      <p
                        class="text-xs text-neutral-600 dark:text-neutral-400 italic"
                      >
                        {resolution.contributingComments.length} contributing comment{resolution
                          .contributingComments.length !== 1
                          ? "s"
                          : ""} selected
                      </p>
                    {/if}
      ...
      <div class="flex flex-col lg:flex-row gap-6 p-4">
        ...
        {#if mysteryObject}
          <div class="lg:w-1/2 flex-grow order-2 lg:order-1">
            <div
              class="p-5 rounded-md bg-neutral-50 dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 shadow-sm"
            >
              <h3
                class="text-md font-medium mb-4 text-neutral-950 dark:text-white"
              >
                Mystery Object Properties
              </h3>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                {#each Object.entries(mysteryObject).filter(([key]) => !skipProperties.includes(key) && !key
                      .toLowerCase()
                      .includes("colorname") && key !== "color_name" && key !== "color_label") as [key, value]}
                  {#if value}
                    <div
                      class="bg-white dark:bg-neutral-950 p-3 rounded-md border border-neutral-100 dark:border-neutral-700 hover:border-neutral-300 dark:hover:border-neutral-600 transition-colors shadow-sm"
                    >
                      <span
                        class="block text-xs font-medium text-neutral-500 dark:text-neutral-400 mb-1 object-part-name"
                      >
                        {key
                          .split(/(?=[A-Z])/)
                          .join(" ")
              ...
              {#if mysteryObjectSubParts && mysteryObjectSubParts.length > 0}
                <div class="mt-6">
                  <h4
                    class="font-medium text-base mb-3 text-neutral-800 dark:text-neutral-300 border-b border-neutral-200 dark:border-neutral-700 pb-2"
                  >
                    Object Parts
                  </h4>
                  <div class="space-y-4">
                    {#each mysteryObjectSubParts as part (part.id)}
                      <div
                        class="bg-white dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-700 rounded-lg p-3 shadow-sm hover:shadow-md transition-shadow duration-200"
                      >
                        <div
                          class="flex justify-between items-center mb-3 border-b border-neutral-100 dark:border-neutral-800 pb-2"
                        >
                          <h5
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                          {#if part.material}
                            <div
                              class="bg-neutral-50 dark:bg-neutral-950 p-2 rounded-md border border-neutral-100 dark:border-neutral-800 hover:border-neutral-300 dark:hover:border-neutral-700 transition-colors"
                            >
                              <span
                          ...
                          {/if}
                          {#if part.color && !Object.keys(part).some((k) => k
                                  .toLowerCase()
                                  .includes("colorname") && k !== "color")}
                            <div
                              class="bg-neutral-50 dark:bg-neutral-950 p-2 rounded-md border border-neutral-100 dark:border-neutral-800 hover:border-neutral-300 dark:hover:border-neutral-700 transition-colors"
                            >
                              <span
                              ...
                              <span class="flex items-center gap-2">
                                ...
                                {:else}
                                  {#await fetchColorHexFromName(part.color)}
                                    <div
                                      class="w-4 h-4 rounded border bg-neutral-200"
                                    ></div>
                                  {:then hexColor}
                                    <div
                                      class="w-4 h-4 rounded border"
                                      style="background-color: {hexColor ||
                                        formatColorForDisplay(part.color)};"
                                    ></div>
                                    <span class="text-sm">{part.color}</span>
                                  {:catch}
                                    <div
                                      class="w-4 h-4 rounded border"
                                      style="background-color: {formatColorForDisplay(
                                        part.color,
                                      )};"
                                    ></div>
                                    <span class="text-sm">{part.color}</span>
        ...
        {#if mediaFiles.length > 0}
          <div class="lg:w-1/2 flex-shrink-0 order-1 lg:order-2">
            <div
              class="relative rounded-md overflow-hidden bg-neutral-50 dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 shadow-sm"
            >
              <!-- Carousel navigation buttons -->
              {#if mediaFiles.length > 1}
                <div
                  class="absolute top-0 bottom-0 left-0 flex items-center z-10"
                >
                  <button
                    on:click={prevMedia}
                ...
                <div
                  class="absolute top-0 bottom-0 right-0 flex items-center z-10"
                >
                  <button
                    on:click={nextMedia}
                ...
                <!-- Media counter -->
                <div
                  class="absolute bottom-4 left-0 right-0 flex justify-center z-10"
                >
                  <div
                    class="bg-black/60 backdrop-blur-sm text-white px-3 py-1 rounded-full text-xs shadow-sm"
                  >
              ...
              <div class="carousel-container w-full">
                {#if mediaFiles[currentMediaIndex]}
                  ...
                  {:else}
                    <div
                      class="flex flex-col items-center justify-center p-10 h-[300px]"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        class="h-24 w-24 text-rose-500 dark:text-rose-400 mb-6"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >
                        <path
                          ...
                          stroke-linejoin="round"
                          stroke-width="2"
                          d="M9 19V6l12-3v13M9 19c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zm12-3c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2zM9 10l12-3"
                        />
                      </svg>
                      <audio src={media.url} controls class="w-full max-w-md">
              ...
              {#if mediaFiles.length > 1}
                <div
                  class="flex justify-center gap-2 px-4 py-3 bg-neutral-100 dark:bg-neutral-950 overflow-x-auto"
                >
                  {#each mediaFiles as media, i}
                    <button
                      ...
                      class="flex-shrink-0 w-14 h-14 rounded-md overflow-hidden focus:outline-none transition-all duration-200 ease-in-out
                        {i === currentMediaIndex
                        ? 'ring-2 ring-teal-500 transform scale-105 shadow-md'
                    ...
                    >
                      ...
                      {:else if media.type === "video"}
                        <div
                          class="w-full h-full bg-neutral-200 dark:bg-neutral-700 flex items-center justify-center"
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                      ...
                      {:else if media.type === "audio"}
                        <div
                          class="w-full h-full bg-neutral-200 dark:bg-neutral-800 flex items-center justify-center"
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
          ...
          <div class="tag-tooltip-body">
            {#if tagImageLoading}
              <div class="tag-image-placeholder">
                <div
                  class="inline-block h-8 w-8 border-4 border-neutral-200 dark:border-neutral-800 border-t-teal-600 dark:border-t-teal-500 rounded-full animate-spin"
                ></div>
              </div>
            {:else if tagImage}
...
{#if showReportPostModal}
  ...
  >
    <div
      class="bg-white dark:bg-neutral-950 rounded-md shadow-xl max-w-md w-full p-6 border border-neutral-200 dark:border-neutral-800"
    >
      ...
      <div class="mb-4">
        ...
        <select
          bind:value={reportPostReason}
          class="w-full p-2 border rounded-md bg-white dark:bg-neutral-800 border-neutral-300 dark:border-neutral-600 text-sm"
        >
      ...
      <div class="mb-4">
        ...
        <Textarea
          rows="4"
          bind:value={reportPostNotes}
          class="w-full p-2 border rounded-md bg-white dark:bg-neutral-800 border-neutral-300 dark:border-neutral-600 text-sm"
          placeholder="Optional details (e.g., link to evidence)..."
        />
      </div>
      ...
      <div class="flex justify-end gap-2">
        <Button
          variant="outline"
          on:click={() => (showReportPostModal = false)}
          class="text-sm"
        >
          Cancel
        </Button>
        <Button
          ...
          on:click={submitPostReport}
          disabled={reportPostSubmitting}
        >
          {reportPostSubmitting ? "Submitting..." : "Submit Report"}
        </Button>
      </div>
    </div>
  </div>
